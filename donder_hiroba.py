...
def access_ranking(s_num, p):
    ...
    for score in Soup.find_all("div", attrs={"class": "rankingDetailScore"}):#全国ランキングの詳しいスコア
        ...
        #print(score_num)
        ranking_data[s_num-1][(p-1)*10 + idx] = score_num
        idx += 1
...
my_score_data = []
ranking_data = [["0"] * 100 for _ in range(song_no_max)] #全ての曲の成績を10000個
'''
...
for s_num in range(1,song_no_max+1):
    for p in range(1,page_max+1):
        print("page is",p,"song_no is",s_num)
        access_ranking(s_num, p)
...
with open(path, mode='x') as f:
    for i in range(song_no_max):
        title = "\n   Song_Number is "+str(i+1)+"\n"
        f.write(title)
        f.write('\n'.join(ranking_data[i]))
